...
j_val, J2, J3, lmbda, alpha0 = 40.0, 0.1, 0.1, 0.1, -0.1
u_val = 0.5
H = 10
...
# Setup variables and functions to solve the PDEs
...
while(count < max_iter):
    ...
    data = HDF5File(mesh.mpi_comm(), f'{data_dir}/data_N_{N}_seed_{seed}_alpha0_{alpha0}_H_{H}_t_{t_total}_J1_{j_val}_J2_{J2}_J3_{J3}_u_bar_{u_val}_count_{count}_length_{length}_width_{width}.h5', "w")
    ...
    if(count > 0 == 0):
        ...
        if(rank == 0):
            np.savez(f"{data_dir}/cost_N_{N}_seed_{seed}_alpha0_{alpha0}_H_{H}_t_{t_total}_J1_{j_val}_J2_{J2}_J3_{J3}_u_bar_{u_val}_count_{count}_length_{length}_width_{width}.npz", cost = total_cost, lr = learning)
